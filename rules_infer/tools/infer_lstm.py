...
class SocialInteractionDetector:
    ...
    def _get_agent_trajectories_in_scene(self, scene_token):
        ...
        while current_sample_token:
            ...
            for ann_token in sample['anns']:
                ...
                agent_data[ann['instance_token']]['timestamps'].append(sample['timestamp'])
                agent_data[ann['instance_token']]['positions'].append(ann['translation'][:2])
                agent_data[ann['instance_token']]['rotations'].append(ann['rotation'])
                agent_data[ann['instance_token']]['category_name'].append(ann['category_name'])
            current_sample_token = sample['next']
        processed_agent_data = {}
        for instance_token, data in agent_data.items():
            if len(data['timestamps']) < self.config['HIST_LEN'] + self.config['PRED_LEN']: continue
            data_np = {'timestamps': np.array(data['timestamps']) / 1e6, 'positions': np.array(data['positions']),
                       'rotations': data['rotations'], 'category_name': data['category_name'][0]}
            data_np['velocities'], data_np['accelerations'] = calculate_kinematics(data_np['positions'], data_np['timestamps'])
    ...
    def _calculate_trigger_metrics(self, pred_traj, gt_data):
        ...
        gt_vel, gt_accel = gt_data['velocities'], gt_data['accelerations']
        vel_error, accel_error = np.linalg.norm(pred_vel - gt_vel, axis=1), np.linalg.norm(pred_accel - gt_accel,
                                                                                           axis=1)
        long_vel_errors, lat_vel_errors = [], []
        for i in range(len(gt_data['rotations'])):
            q = Quaternion(gt_data['rotations'][i]);
            heading_vec = q.rotation_matrix[:2, 0]
            v_error_vec = gt_vel[i] - pred_vel[i];
            long_error = np.dot(v_error_vec, heading_vec)
            lat_vel_errors.append(np.linalg.norm(v_error_vec - long_error * heading_vec))
            long_vel_errors.append(long_error)
        return {"ADE": ade, "FDE": fde, "max_velocity_error": np.max(vel_error) if vel_error.size > 0 else 0,
    ...
    def _calculate_attribution_metrics(self, primary_data, alter_data, event_start_idx, pred_len, primary_pred_traj):
        ...
        alter_indices = \
        np.where((alter_data['timestamps'] >= ts_primary_start) & (alter_data['timestamps'] <= ts_primary_end))[0]
        if len(alter_indices) == 0: return None
        primary_pos_event, alter_pos_event = primary_data['positions'][event_start_idx:event_end_idx], \
        alter_data['positions'][alter_indices]
        min_dist = np.min(
        ...
        pos_p, vel_p = primary_data['positions'][pre_event_idx_p], primary_data['velocities'][pre_event_idx_p]
        pos_a, vel_a = alter_data['positions'][pre_event_idx_a], alter_data['velocities'][pre_event_idx_a]
        rel_pos, rel_vel = pos_a - pos_p, vel_a - vel_p
        min_ttc = float('inf')
        if np.dot(rel_pos, rel_vel) < 0 and np.dot(rel_vel, rel_vel) > 1e-6:
            rel_speed_val = np.linalg.norm(vel_p - vel_a)
            if rel_speed_val > 0.1: min_ttc = np.linalg.norm(rel_pos) / rel_speed_val
        path_conflict = False;
        ...
        for i in range(len(primary_pred_traj) - 1):
            ...
            for j in range(len(alter_path_segments) - 1):
                ...
                if check_line_segment_intersection(p1, p2, p3, p4): path_conflict = True; break
            if path_conflict: break
        heading_q = Quaternion(primary_data['rotations'][pre_event_idx_p])
        heading_vec, side_vec = heading_q.rotation_matrix[:2, 0], heading_q.rotation_matrix[:2, 1]
        long_dist, lat_dist = np.dot(rel_pos, heading_vec), np.dot(rel_pos, side_vec)
        pos_desc = "front" if long_dist > 0 else "rear";
        if abs(lat_dist) > 2: pos_desc += "-left" if lat_dist > 0 else "-right"
        return {"min_distance": min_dist, "min_TTC_pre_event": min_ttc if min_ttc != float('inf') else None,
                "predicted_path_conflict": path_conflict, "relative_position": pos_desc}

    def analyze_scene(self, scene_token):
        all_agent_data = self._get_agent_trajectories_in_scene(scene_token)
        detected_events = []
        H, P = self.config['HIST_LEN'], self.config['PRED_LEN']
        num_agents_to_analyze, agent_count = len(all_agent_data), 0

        for primary_id, primary_data in all_agent_data.items():
            agent_count += 1
            print(f"  -> Analyzing agents: [{agent_count}/{num_agents_to_analyze}]", end='\r')
            if 'vehicle' not in primary_data['category_name']: continue
            num_timesteps = len(primary_data['timestamps'])
            for i in range(num_timesteps - H - P + 1):
                hist_end_idx = i + H
                last_hist_pos = primary_data['positions'][i:hist_end_idx][-1]
                normalized_hist = primary_data['positions'][i:hist_end_idx] - last_hist_pos
                input_tensor = torch.from_numpy(normalized_hist).float().unsqueeze(0).to(self.device)
                with torch.no_grad():
                    pred_abs_traj = self.model(input_tensor).squeeze(0).cpu().numpy() + last_hist_pos
                ...
                if is_event:
                    ...
                    for alter_id, alter_data in all_agent_data.items():
                        if alter_id == primary_id: continue

                        # [核心修复] 只考虑动态物体作为交互对象
                        if alter_data['category_name'] not in DYNAMIC_CATEGORIES:
                            continue
                        ...
                        if attribution_metrics:
                            score = 0
                            if attribution_metrics['min_distance'] < 10: score += 1.0 / (
                                        attribution_metrics['min_distance'] + 1e-6)
                            if attribution_metrics['min_TTC_pre_event'] and attribution_metrics[
                                'min_TTC_pre_event'] < 5: score += 2.0 / (
                                        attribution_metrics['min_TTC_pre_event'] + 1e-6)
                            if attribution_metrics['predicted_path_conflict']: score += 5.0
                            if score > 0.1:
                                candidate_agents.append(
                                    {"agent_id": alter_id, "agent_type": alter_data['category_name'],
                                     "interaction_score": score, "attribution_metrics": attribution_metrics})
                    if not candidate_agents: continue
                    ...
                    detected_events.append({
                        ...
                        "primary_agent": {"agent_id": primary_id, "agent_type": primary_data['category_name'],
