...
def render_annotation_in_image(nusc: NuScenes,
                               ...
                               thickness=2):
    """
    一个辅助函数，将单个nuScenes标注渲染到指定相机的OpenCV图像上。
    """
    # 获取要渲染的3D框
    ...
    # --- 坐标系变换: Global -> Ego Vehicle -> Camera ---

    # 1. 从 Global 坐标系变换到 Ego Vehicle 坐标系
    pose_record = nusc.get('ego_pose', sd_record['ego_pose_token'])
    box.translate(-np.array(pose_record['translation']))
    box.rotate(Quaternion(pose_record['rotation']).inverse)

    # 2. 从 Ego Vehicle 坐标系变换到 Camera 坐标系
    ...
    # 3. 投影到2D图像平面并绘制
    view_camera_intrinsic = np.array(cs_record['camera_intrinsic'])
            ...
            for frame_idx in range(start_frame, end_frame):
                ...
                for cam_name in cam_names:
                    ...
                    for agent_token in interacting_tokens:
                        ann_token = next((ann['token'] for ann in nusc.sample_annotation if
                                          ann['instance_token'] == agent_token and ann['sample_token'] == sample_token),
                                         None)
                        if ann_token:
                            # 调用独立的辅助函数
                            render_annotation_in_image(nusc, ann_token, cam_token, img,
                                                       box_color=INTERACTING_AGENT_COLOR, thickness=BOX_THICKNESS)

                    # 绘制关键 agent (红色)
                    key_ann_token = next((ann['token'] for ann in nusc.sample_annotation if
                                          ann['instance_token'] == key_agent_token and ann[
                                              'sample_token'] == sample_token), None)
                    if key_ann_token:
                        # 调用独立的辅助函数
                        render_annotation_in_image(nusc, key_ann_token, cam_token, img, box_color=KEY_AGENT_COLOR,
                ...
                image_filename = f"frame_{frame_idx:04d}.jpg"
